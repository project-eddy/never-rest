...
| [contract-compilation.spec.md](./contract-compilation.spec.md) | `compileContract`, `assertHandlersComplete`, `matchPath` decode, `isContractPath` — duplicate matchers, trailing slashes, handler completeness, host path membership, params/query/body rules |
| [input-sources.spec.md](./input-sources.spec.md) | Explicit `params` / `query` / `body` on `RouteDef`, `parseRouteSources` — source independence, compile-time rejections |
