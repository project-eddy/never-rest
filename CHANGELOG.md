...
- New `./local` entry point for in-process dispatch of a `ContractDef`. `createLocalClient` gives one typed method per operation; `createDispatcher` exposes the same machinery addressed by operation name, for foreign transports that carry the operation as a string (NDJSON sockets, MCP stdio, agent tool calls). Declared input and output still validate through `parseRouteSources` / `parseOutput`, but no `Request`, `Response`, or JSON round-trip is constructed. Disclosure defaults to `full` because callers share a process, and therefore a trust circle. Route `errors` status maps are ignored — HTTP status is not a local concern. See [specs/local-dispatch.spec.md](specs/local-dispatch.spec.md), [docs/concepts.md](docs/concepts.md#one-contract-more-than-http), and [docs/api.md](docs/api.md#eddy-worksnever-restlocal).
